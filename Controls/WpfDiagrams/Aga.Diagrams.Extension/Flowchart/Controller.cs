...
namespace Aga.Diagrams.Extension.Flowchart
{
    public class Controller : IDiagramController
	{
		...
		public Controller(FrameworkElement editor, DiagramView view, FlowchartModel model)
		{
            _editor = editor;
            _view = view;
			_model = model;
		...
		private Node UpdateNode(FlowNode node, Node item)
		{
			if (item == null)
			{
                ...
                item.Content = CreateContent(_editor, node);
		...
		public static FrameworkElement CreateContent(FrameworkElement parent, FlowNode node)
		{
            ContentControl content = new ContentControl();
            content.DataContext = node;
            switch (node.Kind)
            {
                case NodeKinds.Start:
                    content.Style = parent.FindResource("StartNodeStyle") as Style;
                    break;
                case NodeKinds.End:
                    content.Style = parent.FindResource("StartNodeStyle") as Style;
                    break;                   
                case NodeKinds.Action:
                    content.Style = parent.FindResource("ActionNodeStyle") as Style;
                    break;
                case NodeKinds.Condition:
                    content.Style = parent.FindResource("ConditionNodeStyle") as Style;
                    break;
                case NodeKinds.Middle:
                    content.Style = parent.FindResource("MiddleNodeStyle") as Style;
                    break;
                case NodeKinds.Decide:
                    content.Style = parent.FindResource("DecideNodeStyle") as Style;
                    break;
                case NodeKinds.COBegin:
                    content.Style = parent.FindResource("COBeginNodeStyle") as Style;
                    break;
                case NodeKinds.COEnd:
                    content.Style = parent.FindResource("COEndNodeStyle") as Style;
                    break;
                case NodeKinds.Normal:
                    content.Style = parent.FindResource("NormalNodeStyle") as Style;
                    break;
            }
            return content;
		...
		{
			var inode = _view.Items.FirstOrDefault(p => p.ModelElement == node) as INode;
			if (inode == null)
				return null;
