...
namespace Aga.Diagrams
{
	public class DiagramView : Canvas
	{
        ...
        #region IsThemeChanged

        public static readonly DependencyProperty IsThemeChangedProperty =
            DependencyProperty.Register("IsThemeChanged",
                                       typeof(bool),
                                       typeof(DiagramView),
                                       new FrameworkPropertyMetadata(false, new PropertyChangedCallback(IsThemeChangedChanged)));

        private static void IsThemeChangedChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
        {
            var view = d as DiagramView;
		...
		{
            var lightGray = Application.Current.FindResource("MahApps.Brushes.Gray8") as SolidColorBrush;
			return new Pen(lightGray??Brushes.LightGray, (1 / Zoom));
		}
		...
		{
            if (Mode != "Edit") { e.CanExecute = false; return; }
            if (Controller != null)
